...
- 完整仓库测试已在本地实际执行，当前为 **`212 passed in 21.37s`**，不再沿用
  静态记录的 `187 passed`。
...
4. CLI 已新增 `--mshnet-variant {workbench,official,deterministic}`，clean variant
   物理上不包含 DEA-lite head 或 `fusion_alpha`。为兼容尚在运行的历史 checkpoint，
   当前默认仍为 `workbench`；clean deterministic 400-epoch baseline 尚未重跑，故
   Gate 0 还不能标记为完全闭环。
...
复跑，并实际运行 M3/M4/normalization controls。只有性能和 optimizer/component 证据
